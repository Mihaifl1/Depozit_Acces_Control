/* esp_vestiar_complete_dual_tz.ino
   ESP8266 NodeMCU ? Vestiar COMPLET (actualizat)
   - Wiegand (keypad + RFID)
   - Relay control (puls / hold + autolock 3s pt card/parol?)
   - MQTT (cmd/status/events, cards list, schedule)
   - HTTP direct-to-DB (PHP): event/status/cards/schedule
   - LittleFS (cards, schedule, tz)
   - NTP time (cu fus orar setabil din UI) + print în Serial
   - Web UI (program?ri + carduri + fus orar) cu CORS
   - Status MQTT include: ip, mac, rssi, door, pwr
   - Public? ?i topicul info: locker/<ID>/info (ip, freeHeap)
   - UI cite?te din PHP (vestiar_dbinfo.php) ultimul card + num?r?tori
*/
...
// ---------- CONFIG PHP (HTTP direct-to-DB) ----------
const char* PHP_EVENT_URL    = "http://192.168.66.240/vestiar_event.php";      // evenimente CARD/LOCK/UNLOCK/KEYPAD/etc
const char* PHP_STATUS_URL   = "http://192.168.66.240/vestiar_status.php";     // status periodic (lock/ip/mac/rssi/door/pwr)
const char* PHP_CARDS_ADD    = "http://192.168.66.240/vestiar_cards_add.php";  // ad?ugare card
const char* PHP_CARDS_REMOVE = "http://192.168.66.240/vestiar_cards_remove.php";// ?tergere card
const char* PHP_SCHEDULE_SET = "http://192.168.66.240/vestiar_schedule.php";   // salvare schedule în DB
const char* PHP_DBINFO_URL   = "http://192.168.66.240/vestiar_dbinfo.php";     // UI: ultimul card, totaluri
// --------- CONFIG PHP (cards list direct) ----------
const char* PHP_CARDS_LIST = "http://192.168.66.240/vestiar_cards_list.php"; // -> trebuie s? returneze JSON array: [{ "card":12345, "first":"Ion", "last":"Pop" }, ...]
...
const unsigned long RELAY_PULSE_MS = 0; // 0 = hold (f?r? puls)
...
const char* FILE_TZ        = "/tz.json";        // nou ? fus orar
...
// Time / NTP
long tz_offset = 2 * 3600;       // nou ? variabil (UTC+2 default)
...
String lockState = "LOCKED"; // LOCKED | UNLOCKED | AUTOLOCK
...
// ---------- TIME / TZ ----------
void applyTimezone(long hours) {
  tz_offset = hours * 3600;
  configTime(tz_offset, DST_OFFSET, NTP1, NTP2);
}
void loadTZ() {
  if (!LittleFS.exists(FILE_TZ)) return;
  File f = LittleFS.open(FILE_TZ, "r"); if (!f) return;
...
void saveTZ(long hours) {
  DynamicJsonDocument d(128);
  d["tz"] = (int)hours;
  File f = LittleFS.open(FILE_TZ, "w");
...
// ---------- UTIL ----------
String timeToStr(time_t ts) { struct tm t; localtime_r(&ts, &t); char buf[32]; strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S", &t); return String(buf); }
String nowStamp() { time_t n = time(nullptr); if (n < 100000) return String("NTP not ready"); return timeToStr(n); }
...
String ipStr() { if (WiFi.status() == WL_CONNECTED) return WiFi.localIP().toString(); return String("-"); }


// ---------- HTTP helper (direct-to-DB) ----------
void httpPostJson(const char* url, const String& json) {
...
void fetchCardsFromServer() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("fetchCardsFromServer: WiFi not connected");
    return;
  }
  ...
  HTTPClient http;
  WiFiClient client;
  http.begin(client, PHP_CARDS_LIST);
  int code = http.GET();
  if (code != HTTP_CODE_OK) {
    Serial.printf("fetchCardsFromServer: HTTP GET failed code=%d\n", code);
    http.end();
    return;
  }
  ...
  // Parse JSON into allowedCards
  Serial.printf("fetchCardsFromServer: received %u bytes\n", (unsigned)body.length());
  size_t cap = body.length() + 2048; if (cap > 32768) cap = 32768;
  DynamicJsonDocument doc(cap);
  DeserializationError err = deserializeJson(doc, body);
  if (err) {
    Serial.printf("fetchCardsFromServer: JSON parse err: %s\n", err.c_str());
    return;
  }


  // Clear current list and fill
  allowedCards.clear();
  if (doc.is<JsonArray>()) {
    for (JsonVariant v : doc.as<JsonArray>()) {
      if (v.is<unsigned long>() || v.is<unsigned long long>()) {
        ...
        unsigned long card = (unsigned long)(o["card"] | 0UL);
        String first = String((const char*)(o["first"] | ""));
        String last  = String((const char*)(o["last"]  | ""));
        if (card) { CardEntry e; e.card = card; e.first = first; e.last = last; allowedCards.push_back(e); }
      }
    }
    Serial.printf("fetchCardsFromServer: loaded %u cards\n", (unsigned)allowedCards.size());
    if (!saveCardsToFS()) Serial.println("fetchCardsFromServer: warning saving to FS failed");
  } else {
    Serial.println("fetchCardsFromServer: unexpected JSON (not array)");
  }
}
...
bool parseScheduleJson(const String& json) {
  ...
  for (size_t i = 0; i < arr.size() && rulesCount < MAX_RULES; ++i) {
    ...
    Serial.printf(" parsed rule[%u]: %02d:%02d action=%s note='%s'\n",(unsigned)rulesCount, H, M, R.action.c_str(), R.note.c_str());
    rulesCount++;
  }
...
void logEvent(const char* type, const char* source, const char* extra = nullptr) {
  ...
  doc["ts"] = nowStamp(); doc["type"] = type; doc["source"] = source; if (extra) doc["extra"] = extra; doc["door"] = DOOR_ID;
  publishEventJson_MQTT_and_DB(doc.as<JsonObject>());
}
void logCardEvent(unsigned long card24, unsigned int facility, unsigned int cardNumber, bool ok) {
  DynamicJsonDocument doc(512);
  doc["ts"] = nowStamp(); doc["type"] = "CARD"; doc["ok"] = ok; doc["card24"] = card24; doc["facility"] = facility; doc["cardNumber"] = cardNumber; doc["door"] = DOOR_ID;
...
void logKeypadEvent(const String &code, bool ok) {
  ...
  doc["ts"] = nowStamp(); doc["type"] = "KEYPAD"; doc["ok"] = ok; doc["code"] = code; doc["door"] = DOOR_ID;
  publishEventJson_MQTT_and_DB(doc.as<JsonObject>());
}
...
void setRelayState(bool on, bool byCard = false, bool autoLock = false) {
  ...
  if (on && autoLock) { relayOnSince = millis(); waitingAutoLock = true; }
  else waitingAutoLock = false;


  if (on) relayOnSince = millis(); else waitingAutoLock = false;
  if (autoLock) lockState = "AUTOLOCK"; else lockState = on ? "UNLOCKED" : "LOCKED";
  ...
  // eveniment LOCK/UNLOCK c?tre MQTT + DB
  DynamicJsonDocument d(256);
  d["ts"] = nowStamp(); d["type"] = "LOCK"; d["state"] = lockState; d["door"] = DOOR_ID; d["byCard"] = byCard; d["autoLockArm"] = autoLock;
  publishEventJson_MQTT_and_DB(d.as<JsonObject>());
}
...
void publishStatus() {
  ...
  httpPostJson(PHP_STATUS_URL, out); // trimitem status ?i în DB
}
void publishInfo() {
  ...
  mqtt.publish(topic.c_str(), out.c_str(), true); // retained (doar MQTT e rezonabil aici)
}
...
char mapKeyCode(unsigned long long data, uint8_t bits)
{
  ...
  switch (data)
  {
    ...
    case 75:  return '*';   // ? din log apare 75 la final
    case 60:  return '#';   // ? foarte probabil 60 este #
...
void processWiegand() {
  ...
  if (treatAsCard) {
    ...
    logCardEvent(card24, facility, cardNum, ok); // MQTT + DB
    ...
    if (enrollMode) {
      ...
      String out; serializeJson(d, out);
      // în locul MQTT add imediat ? trimitem ?i în DB:
      httpPostJson(PHP_CARDS_ADD, out);
...
if (ch != '?') {
  ...
  // anti bounce
  if (millis() - lastKeyMs < 80) {
    Serial.printf("Key repeat ignored %c\n", ch);
    return;
  }
  lastKeyMs = millis();
  ...
  if (ch == '#') {
    ...
    Serial.printf("ENTER pressed. Buffer=%s Master=%s\n",
                  keypadBuffer.c_str(),
                  masterCode.c_str());
    ...
    if (ok) {
      Serial.println("PASSWORD OK -> UNLOCK");
      setRelayState(true, true, true);
      publishStatus();
    } else {
  ...
  // CLEAR
    // CLEAR (*)  ? dar dac? parola e corect?, trateaz? ca ENTER/UNLOCK
  if (ch == '*') {


    Serial.printf("(*) pressed. Buffer=%s Master=%s\n",
                  keypadBuffer.c_str(),
                  masterCode.c_str());
    ...
    if (ok) {
      Serial.println("PASSWORD OK (*) -> UNLOCK");
      setRelayState(true, true, true);   // activeaz? releul pe D0 (PIN_RELAY = 16)
      publishStatus();
    } else {
  ...
  DynamicJsonDocument ud(192); ud["ts"] = nowStamp(); ud["type"] = "WG_UNKNOWN"; ud["src"] = src; ud["bits"] = bits; ud["data"] = String((unsigned long long)data); ud["door"] = DOOR_ID;
  publishEventJson_MQTT_and_DB(ud.as<JsonObject>());
}
...
void onMqtt(char* topic, byte* payload, unsigned int len) {
  ...
  if (t == T_SCHED_SET) {
    ...
    if (parseScheduleJson(msg)) {
      saveScheduleFS(msg);
      mqtt.publish(T_SCHED_CUR.c_str(), msg.c_str(), true);
      // trimitem schedule ?i în DB
      httpPostJson(PHP_SCHEDULE_SET, msg);
      logEvent("MQTT", "SCHEDULE_SAVE");
    } else {
      Serial.println("SCHED SET parse failed");
      logEvent("MQTT", "SCHEDULE_PARSE_FAIL");
    }
    return;
  }
...
<title>ESP Vestiar ? Program?ri & Carduri & Or?</title>
<style>
...
.rule{
  display:grid;
  grid-template-columns:30px 1fr 110px 120px 1fr 36px;
  gap:8px;
  align-items:center;
  padding:8px;
...
<body>
<div class="container">
  <h1>ESP Vestiar ? Program?ri & Carduri & Or?</h1>
  ...
  <div class="panel">
    <div class="row">
      ...
      <div style="margin-left:auto"><button id="btnReload" class="button">Reîncarc?</button></div>
    </div>


    <div class="sectionTitle">Set?ri or? (fus orar)</div>
    <div class="row">
      ...
      <button id="saveTz" class="button">Salveaz?</button>
      <span id="tzMsg" class="small"></span>
    </div>
        <div class="sectionTitle">Parol? de acces (Keypad)</div>
...
<div class="row">
  <div style="flex:1;display:flex;flex-direction:column;gap:6px;max-width:220px">
    ...
    <!-- NOU: afi?are parol? curent? mascat -->
    <input id="codeShown" type="text" readonly
           placeholder="Parola actual?: ?"
           style="width:100%;text-align:center;opacity:.9">


    <input id="codeCurrent" type="password" placeholder="Parol? curent? (dac? exist?)" style="width:100%;text-align:center">
    <input id="codeNew" type="password" placeholder="Parol? nou? (4?8 cifre)" style="width:100%;text-align:center">
    <input id="codeConfirm" type="password" placeholder="Confirm? parola nou?" style="width:100%;text-align:center">
  </div>


  <div style="display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;margin-left:8px">
    <button id="saveCode" class="button">Salveaz? cod</button>
    ...
    <div class="footer">
      <button id="btnAdd" class="button">Adaug? regul?</button>
      <button id="btnSave" class="button">Salveaz? în ESP</button>
      <button id="btnClear" class="button">?terge toate</button>
    </div>
    <div class="footer">
      <button id="btnExportCSV" class="button">Export Excel</button>
      <input type="file" id="importCSV" accept=".csv" style="display:none">
      <button id="btnImportCSV" class="button">Import Excel</button>
    </div>
    <div class="note">Selecteaz? zilele, ora ?i ac?iunea. Noile reguli au implicit Luni?Vineri selectate.</div>
  </div>
  ...
  <div class="panel">
    <h3>Carduri (cache)</h3>
    <div class="row">
      <button id="btnCardsRefresh" class="button">Actualizeaz? din server</button>
      <button id="btnAddCard" class="button">Adaug? card manual</button>
    ...
    <table class="table" id="cardsTable">
      <thead><tr><th>Card</th><th>Nume</th><th>Ac?iune</th></tr></thead>
...
<div id="cardPrompt" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);align-items:center;justify-content:center">
  <div style="background:#071018;padding:16px;border-radius:8px;max-width:360px;margin:auto">
    <div style="margin-bottom:8px">Adaug? card</div>
    <div style="display:flex;flex-direction:column;gap:6px">
      <input id="cardNum" placeholder="card numeric">
      <input id="cardFirst" placeholder="prenume (op?ional)">
      <input id="cardLast" placeholder="nume (op?ional)">
      <div style="display:flex;gap:8px;justify-content:flex-end">
        <button id="cardAddOk" class="button">Adaug?</button>
        <button id="cardAddCancel" class="button" style="background:#666">Anuleaz?</button>
      </div>
    </div>
  </div>
</div>
...
<script>
const api = (p, m = 'GET', b = null) => {
  const o = { method: m, headers: {} };
  if (b) { o.headers['Content-Type'] = 'application/json'; o.body = JSON.stringify(b); }
  return fetch(p, o).then(r => r.ok ? r.json() : r.text().then(t => { throw t }));
};


// <<< SCHIMB? cu IP-ul serverului t?u PHP >>>
...
function makeRuleElement(r, idx){
  ...
  t.type='time'; 
  t.value=r.time||'08:00'; 
  t.oninput=()=>r.time=t.value;


  const sel=document.createElement('select'); 
  ['UNLOCK','LOCK'].forEach(a=>{
    const o=document.createElement('option'); 
    o.value=a; 
    o.textContent=a; 
    sel.appendChild(o);
  }); 
  sel.value=r.action||'UNLOCK'; 
  sel.onchange=()=>r.action=sel.value;


  const note=document.createElement('input'); 
  note.type='text'; 
  note.value=r.note||''; 
  note.placeholder='not? (op?ional)'; 
  ...
  del.className='del'; 
  del.textContent='?'; 
  del.onclick=()=>{rules.splice(idx,1); renderRules();};
...
document.getElementById('btnExportCSV').onclick = () => {
  let csv = "Nr,Zile,Ora,Ac?iune,Not?\n";
  rules.forEach((r, i) => {
    const zile = (r.days || []).map(d => dayNames[d]).join(' ');
    csv += `${i + 1},"${zile}",${r.time},${r.action},"${(r.note || '').replace(/"/g, '""')}"\n`;
  });
...
document.getElementById('importCSV').addEventListener('change', e => {
  ...
  reader.onload = evt => {
    ...
    const startIdx = rows[0].toLowerCase().startsWith('nr,') ? 1 : 0;
    rules = [];
    for (let i = startIdx; i < rows.length; i++) {
      ...
      parts.push(cur);
      const daysText = (parts[1] || '').trim().replace(/^"|"$/g, '');
      const time     = (parts[2] || '08:00').trim().replace(/^"|"$/g, '');
      const action   = (parts[3] || 'UNLOCK').trim().toUpperCase().replace(/^"|"$/g, '');
      const note     = (parts[4] || '').trim().replace(/^"|"$/g, '');
      const days = [];
      dayNames.forEach((n, d) => { if (daysText.split(/\s+/).includes(n)) days.push(d); });
      const m = time.match(/^(\d{1,2}):(\d{2})$/);
      const hh = m ? String(Math.min(23, Math.max(0, parseInt(m[1], 10)))).padStart(2, '0') : '08';
      const mm = m ? String(Math.min(59, Math.max(0, parseInt(m[2], 10)))).padStart(2, '0') : '00';
      rules.push({ days, time: `${hh}:${mm}`, action: (action === 'LOCK' ? 'LOCK' : 'UNLOCK'), note });
    }
    renderRules();
  };
...
document.getElementById('saveTz').onclick=()=>{ 
  const v=Number(document.getElementById('tzSelect').value);
...
function renderCards(list){
  ...
  list.forEach(c=>{
    ...
    const del=document.createElement('button'); del.textContent='?terge'; del.className='button'; del.onclick=()=> removeCard(c.card);
    td3.appendChild(del);
...
function removeCard(card){
  if (!confirm('?terge card ' + card + '?')) return;
...
document.getElementById('saveCode').onclick = () => {
  ...
  .then(j => {
    if (j.ok) {
      ...
      document.getElementById('codeMsg').textContent = '? Eroare: ' + (j.err || 'necunoscut?');
...
// ---------- WEB API helpers ----------
// ---------- handleRoot: serve?te pagina local din LittleFS ----------


void handleRoot() {
  Serial.println("[HTTP] Servesc interfa?a din LittleFS...");
  ...
  if (LittleFS.exists("/index.html")) {
    ...
    html = f.readString();
    f.close();
    Serial.println("? Servesc /index.html din LittleFS");
  } else {
    html = FPSTR(index_html);
    Serial.println("?? Fallback: folosesc versiunea încorporat? (PROGMEM)");
  }


  // Înlocuie?te variabila din HTML cu IP-ul t?u PHP
  html.replace("%PHP_DBINFO_URL%", PHP_DBINFO_URL);
...
void handleNow() {
  ...
  if (now < 100000) {
    server.send(200, "application/json", "{\"now\":\"NTP not ready\"}");
    return;
  }
  struct tm t;
  localtime_r(&now, &t);
...
void handleStatus() {
  String door = (digitalRead(PIN_DOOR) == LOW) ? "CLOSED" : "OPEN";
  String pwr  = (digitalRead(PIN_PWR)  == HIGH) ? "ON" : "OFF";
  String json = "{\"lock\":\"" + lockState + "\",\"door\":\"" + door + "\",\"pwr\":\"" + pwr + "\"}";
  server.sendHeader("Access-Control-Allow-Origin", "*");
  server.send(200, "application/json", json);
}
...
// ---------- Trimite evenimente CARD la serverul PHP (p?strat pentru compat) ----------
void sendEventToServer(unsigned long card24, unsigned int facility, unsigned int cardNumber, bool ok) {
  ...
  httpPostJson(PHP_EVENT_URL, json);   // direct DB
  if (T_EVENT.length()) mqtt.publish(T_EVENT.c_str(), json.c_str(), false); // ?i MQTT
}
...
void handlePostCardsAdd() {
  ...
  // MQTT (cerere c?tre server)
  if (T_CARDS_ADD.length()) mqtt.publish(T_CARDS_ADD.c_str(), payload.c_str(), false);
  // DB direct
  httpPostJson(PHP_CARDS_ADD, payload);
...
void handlePostCardsRemove() {
  ...
  DynamicJsonDocument p(256); p["card"] = card; String py; serializeJson(p, py);
  if (T_CARDS_REMOVE.length()) mqtt.publish(T_CARDS_REMOVE.c_str(), py.c_str(), false);
  httpPostJson(PHP_CARDS_REMOVE, py);
...
void handlePostSchedule() {
  ...
  httpPostJson(PHP_SCHEDULE_SET, body);
  logEvent("HTTP", "SCHEDULE_SAVE");
  sendJsonWithCORS(200, "{\"ok\":true}");
}


// ---------- Cmd API (LOCK/UNLOCK manual) ----------
void handleCmd() {
  if (!server.hasArg("plain")) { sendJsonWithCORS(400, "{\"ok\":false}"); return; }
  String body = server.arg("plain");
  DynamicJsonDocument d(256); 
  ...
  if (cmd == "UNLOCK") { setRelayState(true, false, false); publishStatus(); logEvent("HTTP", "UNLOCK"); sendJsonWithCORS(200, "{\"ok\":true}"); }
  else if (cmd == "LOCK") { setRelayState(false, false, false); publishStatus(); logEvent("HTTP", "LOCK"); sendJsonWithCORS(200, "{\"ok\":true}"); }
  else sendJsonWithCORS(400, "{\"ok\":false}");
}
void handleOptions() {
  server.sendHeader("Access-Control-Allow-Origin", "*");
...
bool loadCode();
void registerCodeApi();


void setup() {
  keypadBuffer = "";
  Serial.begin(115200); delay(10);
  Serial.println("\n=== ESP Vestiar Complete (MQTT + DB + TZ) ===");
  ...
  // IMPORTANT: GPIO15 (D8) trebuie LOW la boot; rezistor 10k la GND.
  ...
  loadTZ();                 // încarc? TZ
  loadCardsFromFS();        // încarc? carduri
  loadCode();               // încarc? parola tastaturii (Keypad Master Code)
  
  if (LittleFS.exists(FILE_SCHEDULE)) {
    File f = LittleFS.open(FILE_SCHEDULE, "r");
    if (f) {
      String s = f.readString(); f.close();
      if (!parseScheduleJson(s))
        Serial.println("Saved schedule parse failed");
      else
        Serial.println("Loaded schedule from FS");
    }
  } else {
    DynamicJsonDocument doc(512);
    JsonArray arr = doc.createNestedArray("rules");
  ...
  // dup? WiFi OK
  fetchCardsFromServer(); // ia lista ini?ial? direct din DB
...
ArduinoOTA.setPassword("12345");       // parol? pentru securitate (op?ional)
ArduinoOTA.onStart([]() {
  String type;
...
ArduinoOTA.begin();
Serial.println("OTA ready, update available via Wi-Fi!");
  ...
  server.on("/api/tz", HTTP_OPTIONS, handleOptions);


  registerCodeApi();  // înregistreaz? rutele pentru /api/code
  loadCode();         // încarc? codul master din LittleFS
...
void loop() {
  ...
  // auto-lock dup? 3s pentru card/parol?
  if (relayAutoLockActive && millis() - lastRelayOn >= 3000UL) {
    ...
    lockState = "LOCKED";
    Serial.println("Relay AUTO-LOCK dup? 3s (card/parol?)");
    publishStatus();
    logEvent("AUTOLOCK", "FW");
  }
  ...
  // publish status când se schimb? senzori
  ...
  // re-trimite info la schimb?ri IP/RSSI/freeHeap
  ...
  if (millis() - lastSecondPrint > 1000) { lastSecondPrint = millis(); time_t now = time(nullptr); if (now > 100000) ; }
